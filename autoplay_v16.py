...
class Config:
    ...
    OFFERED_MELD_TILE_MATCHING_THRESHOLD = 0.92
    DISCARD_MELD_TILE_MATCHING_THRESHOLD = 0.88
...
class Utils:
    ...
    @staticmethod
    def is_new_on_screen(frame1, frame2=None, screen_type='small_discards', threshold=Config.DISCARD_MELD_TILE_MATCHING_THRESHOLD, overlap_threshold=Config.TILE_OVERLAP_THRESHOLD):
    ...
    @staticmethod
    def find_offered_meld(frame, screen_type='large_discards', threshold=Config.OFFERED_MELD_TILE_MATCHING_THRESHOLD):
        frame1 = frame.copy()
        frame1[0:210, :, :] = 0
        frame1[950:, :, :] = 0
        frame1[1390:, :, :] = 0
    ...
    @staticmethod
    def find_all_discards_and_melds(frame, screen_type='small_discards', threshold=Config.DISCARD_MELD_TILE_MATCHING_THRESHOLD, save_screenshot=False):
        ...
        filename = f'{ms_ts_id}_discard_meld_frame.png'
    ...
    @staticmethod
    def save_png_frame_changes(frame1, frame2, threshold=Config.CHANGE_THRESHOLD_MELDS_FLOWERS, frame_type='normal', save_contours=False):
        ms_ts_id = int(time.time() * 1000)

        def preprocess(frame1, frame_type):
            if frame_type == 'normal':
                ...
                frame1[:,0:450,:] = 1
                frame1[:,2100:,:] = 1
                frame1[350:420,350:1200,:] = 1

            return frame1

        frame1 = preprocess(frame1, frame_type)

        frame2 = preprocess(frame2, frame_type)
        ...
        # Save the new image
        _output_path = f'{Config.SCREENSHOT_PATH}/{Config.TIMESTAMP}/{ms_ts_id}_new_frame.png'
        cv2.imwrite(_output_path, frame2)
        print(f'finished saving to {_output_path}')
                ...
                if gas:
                    ...
                    if screen_type == 'YourDiscard':
                        ...
                        if your_discard_queue.length() > 1:
                            ...
                            Utils.save_png_frame_changes(previous_game_frame, game_frame, frame_type='normal', save_contours=False)
