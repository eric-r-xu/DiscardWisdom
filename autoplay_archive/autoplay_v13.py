...
class GameScreenshotDiscardSaveTask(Thread):
    def __init__(self, game_frame_queue, motion_detector, stop_event, discard_queue):
        super().__init__()
        self.game_frame_queue = game_frame_queue
        self.motion_detector = motion_detector
        self.stop_event = stop_event
        self.discard_queue = discard_queue  
        ...
        while not self.stop_event.is_set():
            ...
            if is_game_frame:
                if previous_frame is not None:
                    detected, _f1, _f2 = self.motion_detector.detect(previous_frame, frame)

                    if detected:
                        self.discard_queue.put(frame)
                        Utils.save_screenshot(frame, prefix='discardSaveTask_')
                else:
                    self.discard_queue.put(frame)
                    Utils.save_screenshot(frame, prefix='discardSaveTask_')
...
if __name__ == "__main__":
    ...
    discard_queue = queue.Queue(maxsize=20)


    game_screenshot_discard_save_task = GameScreenshotDiscardSaveTask(game_frame_queue, motion_detector, stop_event, discard_queue)
    game_screenshot_discard_save_task.daemon = True
    game_screenshot_discard_save_task.start()
            ...
            for screen_type in Config.GAME_ACTION_SCREENS:
                ...
                if r:
                    ...
                    def dict_diff(dict1, dict2):
                        ...
                        for key in dict2:
                            if key not in dict1:
                                diff[key] = ('new', dict2[key])
                            elif dict2[key] != dict1[key]:
                                diff[key] = ('value', dict2[key], dict1[key])

                        for key in dict1:
                            if key not in dict2:
                                diff[key] = ('old', dict1[key])
                    ...
                    if your_turn_queue.length() == 2:
                        ...
                        if len(small_discards_diff) > 0:
                            for k,v in small_discards_diff.items():
                                msg = f"""small discards: {k}: {v}"""
                                print(msg)
                                Notifier.notify(msg)
                        if len(large_discards_diff) > 0:
                            for k,v in large_discards_diff.items():
                                msg = f"""large discards: {k}: {v}"""
                                print(msg)
                                Notifier.notify(msg)

                    # new addition with discard queue
                    dql = discard_queue.qsize()
                    msg = f'dql = {dql}'
                    Notifier.notify(msg)
                    if dql >= 2:
                        for i in range(1, dql):
                            sd1,sd2,ld1,ld2 = Utils.highlight_frame_changes(list(discard_queue.queue)[i-1], list(discard_queue.queue)[i])
                            large_discards_diff = dict_diff(ld1, ld2)
                            small_discards_diff = dict_diff(sd1, sd2)
                            if len(small_discards_diff) > 0:
                                for k,v in small_discards_diff.items():
                                    msg = f"""NEW * small discards: {k}: {v}"""
                                    print(msg)
                                    Notifier.notify(msg)
                            if len(large_discards_diff) > 0:
                                for k,v in large_discards_diff.items():
                                    msg = f"""NEW * large discards: {k}: {v}"""
                                    print(msg)
                                    Notifier.notify(msg)

                        Utils.clear_queue(discard_queue)
                        msg = 'cleared discard queue'
                        print(msg)
                        Notifier.notify(msg)
                    #############################################
