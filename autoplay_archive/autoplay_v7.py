...
class Config:
    ...
    NO_MOTION_CLICK_THRESHOLD = 0.05     # seconds
    CLICK_COORDINATES = {
        "reject": (1220, 430),
        "wall_tile": (1100, 540),
        "D13": (980, 540),
    ...
    TEMPLATE_CLICK_ORDER = defaultdict(str)
    TEMPLATE_CLICK_ORDER['Ad'] = ['x2','x','closed_ad']
    ...
    YOUR_DISCARD_CHOW_SELECTION_CLICK_ORDER = ['wall_tile', 'D13', 'D12', 'D11','D10', 'D9', 'D8', 'D7', 'D6', 'D5', 'D4', 'D3', 'D2', 'D1']
    for k in YOUR_DISCARD_CHOW_SELECTION_CLICK_ORDER:
...
class Utils:
    ...
    def is_screen(
        ...
        for template_path in png_files:
            ...
            # if match found
            if match_found == 1:
                print(f'match found w/ {folder_path.split('/')[-2]}/{template_path}')
...
while (time.time() - start_time) < Config.TIME_LIMIT:
    ...
    if is_game_frame:
        ...
        if is_next_game_frame:
            ...
            non_game_frame_queue.enqueue(frame)
            is_ad_frame, match_locations = Utils.is_screen(frame, type='Ad')
            if is_ad_frame:
                Utils.save_screenshot(frame, prefix='NextGame_')
