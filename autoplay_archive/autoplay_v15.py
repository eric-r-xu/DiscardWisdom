...
class Utils:
    ...
    @staticmethod
    def find_all_discards_and_melds(frame, screen_type='small_discards', threshold=Config.DISCARD_MELD_TILE_MATCHING_THRESHOLD):
    ...
    @staticmethod
    def save_png_frame_changes(frame1, frame2, threshold=Config.TILE_OVERLAP_THRESHOLD):
        ms_ts_id = int(time.time() * 1000)
        ...
        for i, contour in enumerate(contours):
            x, y, w, h = cv2.boundingRect(contour)
            cropped_change = frame2[y:y+h, x:x+w]
            crop_output_path = f'{Config.SCREENSHOT_PATH}/{Config.TIMESTAMP}/{ms_ts_id}_countour_{i}.png'
            cv2.imwrite(crop_output_path, cropped_change)
            print(f'Cropped change saved to {crop_output_path}')
                ...
                if gas:
                    ...
                    if screen_type == 'YourDiscard':
                        ...
                        if your_discard_queue.length() > 1:
                            ...
                            Utils.save_png_frame_changes(previous_game_frame, game_frame)
